...
export const matchAnything = Symbol('matchAnything')
...
type KeyObject = Array<PathPart> & {
  endsWith: (...segments: Array<string>) => boolean
  match: (...segments: Array<RegExp | typeof matchAnything | boolean | number | string>) => boolean
  node: () => string
  parent: () => string
  parents: () => string
...
const buildKeyObject = (pathParts: Array<PathPart>): KeyObject => {
  const buildSimple = () => {
  ...
  const checkMatch = (...segments: Array<RegExp | typeof matchAnything | boolean | number | string>): boolean => {
  ...
  const checkStartsWith = (...segments: Array<string>) => {
    return checkMatch(...segments, matchAnything)
  }
  const checkEndsWith = (...segments: Array<string>) => {
    return checkMatch(matchAnything, ...segments)
  }
  const result = [...pathParts] as KeyObject
  result.endsWith = checkEndsWith
  result.match = checkMatch
  result.node = getNode
  result.parent = getParent
  result.parents = getParents
  result.startsWith = checkStartsWith
  result.toArray = buildArray
  result.toLodash = buildLodash
  result.toString = buildSimple
  return result
}
/**
 * @function
 ...
 * @param mapper the function that is called for each entry. The mapper can return actions.delete to skip adding the current entry, return actions.unchanged to keep the value as-is, return actions.rename(newKey) to rename the current key with the original value, or return actions.rename(newKey, newValue) to rename and transform. The mapper should return the new value (which can be different from the input value to transform it).
 * @returns the new object with all changes applied
 */
const mapDeep = (input: Dict<any>, mapper: MapperFunction): Dict<any> => {
  const processValue = (value: any, pathParts: Array<PathPart>): ProcessResult | typeof deletedMarker => {
    ...
    const actions: ActionToolset = {
      ...
      rename(newKey: string, ...args: [any?]) {
        ...
        // because undefined as an argument triggers the default parameter
    ...
    if (mappedValue === unchanged) {
      ...
      if (effectiveValue !== null && typeof effectiveValue === 'object') {
        ...
        for (const [originalKey, childValue] of Object.entries(effectiveValue)) {
          ...
          // Handle rename - call mapper again with new key if revisit is true
    ...
    if (mappedValue !== null && typeof mappedValue === 'object') {
      ...
      const result: Dict<any> = {}
      // eslint-disable-next-line typescript/no-unsafe-argument
      for (const [originalKey, childValue] of Object.entries(mappedValue)) {
        let childKey = originalKey
        let processed = processValue(childValue, [
          ...pathParts, {
            isArray: false,
